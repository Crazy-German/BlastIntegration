...
bool AssetManager::Initialize(const std::filesystem::path& aContentRootPath)
{
	...
	for (auto& [key, assetRegister] : myAssets)
	{
		if (key.string().find("t_default") != std::string::npos)
		{
			if(!myAssets.at(key).myAssetLoaded)
			{
				myAssets.at(key).myAsset = LoadAsset(myAssets.at(key));
				myAssets.at(key).myAssetLoaded = true;
			}
		}
	}
	return true;
...
bool AssetManager::RegisterAsset(const std::filesystem::path& aPath)
{
	...
	if (ext.ends_with("fbx") || ext.ends_with("FBX"))
	{
		...
		if (/*filename.rfind(u8"UBX_", 0) == 0 || filename.rfind(u8"USP_", 0) == 0 || filename.rfind(u8"UCP_", 0) == 0 || */filename.starts_with('U'))
		{
			return RegisterColliderAsset(assetPath);
		}
	}
	...
	if (ext.ends_with("dds"))
	{
		...
		{
			return RegisterTexture(assetPath);
		}
	}
	if(assetPath.filename().string().starts_with("SC_") || ext.ends_with("shovel"))
	{
		return RegisterScriptAsset(assetPath);
	}
	return false;
}
...
std::shared_ptr<Asset> AssetManager::LoadMesh(const std::filesystem::path& aPath, const bool& aCreateBuffers)
{
	...
	else
	{
		TGA::FBX::Mesh tgaMesh;
		const TGA::FBX::FbxImportStatus importStatus = TGA::FBX::Importer::LoadMesh(myContentRoot / aPath, tgaMesh);
		auto result = GenerateMesh(tgaMesh, aCreateBuffers);
		mesh = result.second;
		if(result.first)
		{
			RegisterColliderAsset(aPath);
		}
	}
	auto asset = std::make_shared<MeshAsset>();
	asset->myMesh = mesh;
...
std::shared_ptr<Asset> AssetManager::LoadAnimation(const std::filesystem::path& aPath) const
{
	...
	else 
	{
		...
		//Needs to be tested with an animation that has events!
		for (auto& eventName : tgaAnimation.EventNames)
		{
			animation->EventNameToIndex.insert({ eventName, static_cast<int>(animation->EventNames.size()) });
			animation->EventNames.push_back(eventName);
		}
		...
		for (const auto& frameData : tgaAnimation.Frames)
		{
			...
			{
				frame.Transforms.emplace(std::pair(i.first, CU::Matrix4x4f(i.second.Data)));
				auto pos = frame.Transforms.at(i.first).GetTranslation();
				CommonUtilities::Quaternionf rot;
				rot = rot.Set(frame.Transforms.at(i.first));
				frame.RealTransforms.emplace(i.first, TransformBase(pos, rot));

			}
			//Needs testing
			for (auto& event : frameData.TriggeredEvents)
			{
				frame.TriggeredEvents.push_back(animation->EventNameToIndex.at(event.first));
			}
			animation->Frames.push_back(frame);
		}
	}
...
bool AssetManager::RegisterColliderAsset(const std::filesystem::path& aPath)
{
	AssetRegister newAsset;

	newAsset.myType = AssetType::ColliderAsset;
	newAsset.myPath = aPath;
	std::string id = toLower(aPath.stem().string());
	if(myAssets.contains(id) && (myAssets.at(id).myType == AssetType::Mesh || myAssets.at(id).myType == AssetType::AnimatedMesh))
	{
		id.replace(id.begin(), id.begin()+id.find_first_of("_"), "U");
		myAssets.emplace(toLower(id), newAsset);
	}
	else
	{
		myAssets.emplace(id, newAsset);
	}
	return true;
}

std::shared_ptr<Asset> AssetManager::LoadCollider(const std::filesystem::path& aPath)
{
	std::shared_ptr<ColliderAsset> asset = std::make_shared<ColliderAsset>();
	auto mesh = std::make_shared<Mesh>();
	if(aPath.extension() == Serialization::format)
	{
		Serializer loader;
	...
	else
	{
		TGA::FBX::Mesh tgaMesh;
		const TGA::FBX::FbxImportStatus importStatus = TGA::FBX::Importer::LoadMesh(myContentRoot / aPath, tgaMesh);
		for(auto& element : tgaMesh.Elements)
		{
			CommonUtilities::Vector3f minPos(element.BoxBounds.Min[0], element.BoxBounds.Min[1], element.BoxBounds.Min[2]);
			CommonUtilities::Vector3f maxPos(element.BoxBounds.Max[0], element.BoxBounds.Max[1], element.BoxBounds.Max[2]);
			if(element.MeshName.starts_with("SM") || element.MeshName.starts_with("SK"))
			{
				continue;
			}
			if(element.MeshName.find("UBX") != std::string::npos)
			{
				
				asset->myColliderAssets.push_back(std::make_shared<BoxColliderData>((maxPos-minPos)*0.5f));
				asset->myColliderAssets.back()->myShapeType = Squish::Box;
				asset->myColliderAssets.back()->aOffset = (maxPos+minPos)*0.5f;
				continue;

			}
			if(element.MeshName.find("USP") != std::string::npos)
			{
				asset->myColliderAssets.push_back(std::make_shared<SphereColliderData>((maxPos.x-minPos.x)*0.5f));
				asset->myColliderAssets.back()->myShapeType = Squish::Sphere;
				asset->myColliderAssets.back()->aOffset = (maxPos+minPos)*0.5f;
				continue;

			}
			
			if(element.MeshName.find("UCP") != std::string::npos)
			{
				float radius = (element.BoxBounds.Max[0]-element.BoxBounds.Min[0])*0.5f;
				float halfHeight = ((element.BoxBounds.Max[1]-element.BoxBounds.Min[1])*0.5f)-radius;
				asset->myColliderAssets.push_back(std::make_shared<CapsuleColliderData>(radius, halfHeight));
				asset->myColliderAssets.back()->myShapeType = Squish::Capsule;
				asset->myColliderAssets.back()->aOffset = (maxPos+minPos)*0.5f;
				continue;

			}
			if(element.MeshName.find("UCX") != std::string::npos)
			{
				asset->myColliderAssets.push_back(std::make_shared<ConvexColliderData>());
				asset->myColliderAssets.back()->myShapeType = Squish::Convex;
				auto collider = std::static_pointer_cast<ConvexColliderData>(asset->myColliderAssets.back());
				for(auto& point : element.Vertices)
				{
					collider->myVerticies.push_back({point.Position[0],point.Position[1], point.Position[2]});
				}
			}
			else //if(element.MeshName.find("UME") != std::string::npos) // this is ony an else as landscapes still don´t have the correct naming
			{
				asset->myColliderAssets.push_back(std::make_shared<MeshColliderData>());
				asset->myColliderAssets.back()->myShapeType = Squish::Mesh;
				auto collider = std::static_pointer_cast<MeshColliderData>(asset->myColliderAssets.back());
				for(auto& point : element.Vertices)
				{
					collider->myVerticies.push_back({point.Position[0],point.Position[1], point.Position[2]});
				}
				for(auto& point : element.Indices)
				{
					collider->myIndiciess.push_back(point);
				}
			}
		}
	}
...
bool AssetManager::RegisterScriptAsset(const std::filesystem::path& aPath)
{
	AssetRegister newAsset;

	newAsset.myType = AssetType::Script;
	newAsset.myPath = aPath;
	std::string id = toLower(aPath.stem().string());
...
std::shared_ptr<Asset> AssetManager::LoadScript(const std::filesystem::path& aPath)
{
	std::shared_ptr<ScriptAsset> script = std::make_shared<ScriptAsset>();
	script->myType = AssetType::Script;
	script->myName = aPath.filename().string();
	std::ifstream inFile(myContentRoot /aPath, std::ios::binary);
	script->myScriptData = std::make_shared<std::vector<uint8_t>>(std::vector<uint8_t>(std::istreambuf_iterator<char>(inFile),std::istreambuf_iterator<char>()));
	inFile.close();
	return script;
}
...
bool AssetManager::RegisterTexture(std::filesystem::path& aPath)
{
	AssetRegister newAsset;
	newAsset.myType = AssetType::Texture;
	newAsset.myPath = aPath;
	newAsset.myAssetLoaded = false;
	myAssets.emplace(toLower(aPath.stem().string()), newAsset);
	return true;
...
void AssetManager::CreateDefaultCollider()
{
	AssetRegister newAsset;
	std::shared_ptr<ColliderAsset> defaultBox = std::make_shared<ColliderAsset>();
	newAsset.myPath = "CollisionBox";
	newAsset.myType = AssetType::ColliderAsset;
	newAsset.myAssetLoaded = true;
	defaultBox->myColliderAssets.push_back(std::make_shared<BoxColliderData>(CommonUtilities::Vector3f(50.f,50.f,50.f)));
	newAsset.myAsset = defaultBox;
	myAssets.emplace(toLower(newAsset.myPath.string()), newAsset);
}
...
{
	std::pair<bool, std::shared_ptr<Mesh>> retVal;
	retVal.first = false;
	Skeleton skeleton;
	...
	{
		if(tgaElement.MeshName.starts_with('U') && tgaElement.MeshName.find("_") != std::string::npos && tgaElement.MeshName.find("_") == 3)
		{
			retVal.first = true;
			continue;
		}
...
std::shared_ptr<Asset> AssetManager::LoadTexture(const std::filesystem::path& aPath)
{
...
std::shared_ptr<Asset> AssetManager::LoadAsset(const AssetRegister& aAssetRegister)
{
	...
	{
		// Checks if the assetregister type is the corect type this is mostly used for coliiders as their data can be inside of a normal SM_ mesh but the data is seperate
		auto filename = assetPath.filename().u8string();
		if (aAssetRegister.myType == AssetType::Animation && filename.rfind(u8"A_", 0) == 0)
		{
			return LoadAnimation(assetPath);
		}
		if ((aAssetRegister.myType == AssetType::Mesh || aAssetRegister.myType == AssetType::AnimatedMesh) && (filename.rfind(u8"SK_", 0) == 0 ||filename.rfind(u8"SM_", 0) == 0))
		{
			return LoadMesh(assetPath);
		}
	// We cant go by the path as this would invalidate all colliders contained inside of normal meshes so we only have te asset type to go of
		if(aAssetRegister.myType == AssetType::ColliderAsset)
		{
			return LoadCollider(assetPath);
		}
	}
	if (ext.ends_with("json"))
	{
