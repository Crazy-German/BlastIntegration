...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_NoSharedDelay                 = 1 << 17,  // IsItemHovered() only: Disable shared delay system where moving from one item to the next keeps the previous myTimer for a short time (standard for tooltips with long delays)
};
...
// Render state is not reset by default because they are many perfectly useful way of altering render state (e.g. changing shaderType/blending settings before an Image call).
