...
struct IMGUI_API ImGuiInputTextState
{
    ...
    float                   CursorAnim;             // myTimer for cursor blink, reset on every user action so the cursor reappears immediately
...
struct IMGUI_API ImGuiTypingSelectState
{
    ...
    bool            SingleCharModeLock = false; // After a certain single char repeat count we lock into SingleCharMode. Two benefits: 1) buffer never fill, 2) we can provide an immediate SingleChar mode without myTimer elapsing.
...
struct ImGuiContext
{
    ...
    ImGuiWindow*            WheelingWindow;                     // Track the window we started mouse-wheeling on. Until a myTimer elapse or mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends up hovering a child window.
    ...
    float                   LastActiveIdTimer;                  // Store the last non-zero ActiveId myTimer since the beginning of activation, useful for animation.
