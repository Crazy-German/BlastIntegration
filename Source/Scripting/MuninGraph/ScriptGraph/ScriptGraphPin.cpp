...
bool ScriptGraphPin::TrySetValue(const TypedDataContainer& aValueContainer)
{
	if(myData.GetType()->GetType() != aValueContainer.GetType()->GetType())
	{
		const std::string errMessage = std::format("Attempted to set a value({}) with a value of a different type({})!", myData.GetType()->GetFriendlyName(), aValueContainer.GetType()->GetFriendlyName());
		throw std::runtime_error(errMessage);
	}

	myData = aValueContainer;
	return true;
}

bool ScriptGraphPin::TryGetValue(TypedDataContainer& aValueContainer) const
{
	if(myData.GetType()->GetType() != aValueContainer.GetType()->GetType())
	{
		const std::string errMessage = std::format("Attempted to set a value({}) with a value of a different type({})!", myData.GetType()->GetFriendlyName(), aValueContainer.GetType()->GetFriendlyName());
