...
std::shared_ptr<ScriptGraphNode> ScriptGraphSchema::CreateNode(const RegisteredNodeClass& aClass)
{
	...
	{
		if(newNode->GetNodeType() == ScriptGraphNodeType::Event || newNode->GetNodeType()== ScriptGraphNodeType::TimerEvent || newNode->GetNodeType() ==ScriptGraphNodeType::CustomEvent)
		{
			ScriptGraphBase* graph = GetMutableGraph();
...
int ScriptGraphSchema::CreateEdge(size_t aSourcePinId, size_t aTargetPinId)
{
	std::string temp;
	if(CanCreateEdge(aSourcePinId, aTargetPinId, temp))
	{
		...
		return CreateEdgeInternal(sourcePin, targetPin);
	}
...
{
	ScriptGraphBase* const graph = GetMutableGraph();
...
bool ScriptGraphSchema::RegisterNode(const std::shared_ptr<ScriptGraphNode>& aNode)
{
	if(aNode)
	{
		const ScriptGraph* sgPtr = dynamic_cast<const ScriptGraph*>(GetGraph());

		if(aNode->HasFlag(ScriptGraphNodeFlag_ScriptGraphOnly) && !sgPtr)
			return false;
	}

	return NodeGraphSchema::RegisterNode(aNode);
}
...
size_t ScriptGraphSchema::CreateDynamicDataPin(size_t aNodeId, std::string_view aLabel, PinDirection aDirection, std::type_index aType)
{
	ScriptGraphBase* const graph = GetMutableGraph();
	std::shared_ptr<ScriptGraphNode> node = graph->myNodes.at(aNodeId);
...
size_t ScriptGraphSchema::CreateDynamicDataPin(ScriptGraphNode* aNode, std::string_view aLabel, PinDirection aDirection, std::type_index aType)
{
...
void ScriptGraphSchema::RecalculateEntryPoints() const
{
	ScriptGraphBase* graph = GetMutableGraph();
	graph->myEntryPoints.clear();
	graph->myNodeIdToEntryPoint.clear();
	for(const auto& node: GetNodes() |std::views::values)
	{
		if(node->GetNodeType() == ScriptGraphNodeType::Event || node->GetNodeType() == ScriptGraphNodeType::TimerEvent || node->GetNodeType() == ScriptGraphNodeType::CustomEvent)
		{
			const auto& newNodeUID = AsObjectUIDSharedPtr(node);
			const std::shared_ptr<ScriptGraphEventNode>& newEventNode = std::static_pointer_cast<ScriptGraphEventNode>(node);
			std::string entryHandle(newEventNode->GetEntryHandle());
...
int ScriptGraphSchema::CreateEdgeInternal(ScriptGraphPin& aSourcePin, ScriptGraphPin& aTargetPin) const
{
	ScriptGraphBase* const graph = GetMutableGraph();
	...
	if(aTargetPin.GetOwner()->GetNodeType() == ScriptGraphNodeType::TimerEvent)
	{
		RecalculateEntryPoints();
	}
	assert(aTargetPin.GetNumConnections() == 1);
	assert(aSourcePin.GetNumConnections() >= 1);
	return (int)newEdgeUID;
}