...
protected:
	...
	{
		if(const auto pinLabelIt = myPinLabelToPinID.find(aPinLabel); pinLabelIt != myPinLabelToPinID.end() && aPinLabel != aNewLabel)
		{
	...
	NodePinClass& GetMutablePin(std::string_view aPinLabel)
	{
		...
		return GetMutablePin(it->second);
	}

public:
	virtual ~NodeGraphNode() = default;

	FORCEINLINE NodeGraphClass* GetOwner() const { return myOwner; }
	...
	FORCEINLINE const std::map<size_t, NodePinClass>& GetPins() const { return myPins; }
	...
	{
		if(myMovedLastFrame == false)
		{
			myPrevPosition[0] = myPosition[0];
			myPrevPosition[1] = myPosition[1];
			myPrevPosition[2] = myPosition[2];
		}
		myPosition[0] = x;
		myPosition[1] = y;
		myPosition[2] = z;
		myMovedLastFrame = myMoved;
		myMoved = true;
	}
	...
	void GetPrevNodePosition(float& outX, float& outY, float& outZ) const
	{
		outX = myPrevPosition[0];
		outY = myPrevPosition[1];
		outZ = myPrevPosition[2];
	}
	bool WasMoved() const {return myMoved || myMovedLastFrame;}
	void ResetMoved()
	{
		myMoved = false;
	}
	void ResetLastFrameMove()
	{
		myMovedLastFrame = false;
	}
private:

	std::map<size_t, NodePinClass> myPins;
	std::unordered_map<std::string, size_t, string_hash, std::equal_to<>> myPinLabelToPinID;

	NodeGraphClass* myOwner = nullptr;

	float myPosition[3] = { 0, 0, 0 };
	float myPrevPosition[3] = { 0, 0, 0 };
	bool myMoved = false;
	bool myMovedLastFrame = false;
