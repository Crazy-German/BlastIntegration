...
float4 main(QuadVStoPS pixel) : SV_TARGET
{
    ...
    {
        [flatten]
        if (SpotLights[index].Active == true)
        {
            ...
            float3 L = SpotLights[0].Lightpos.rgb - worldPos;
            float d = length(L);
            float spotCone = pow(saturate(dot(normalize(L), normalize(SpotLights[index].LightDir.rgb))), SpotLights[index].OuterConeAngle);
            ...
            const float3 lightDirection = normalize(SpotLights[index].Lightpos.rgb - worldPos);
            ...
            //float inShadow = CalculateShadow(worldPos, SpotLights[index].LightViewInv, SpotLights[index].LightProj);

	// resulting color
            float3 radiance = ((diffuseLight + specularLight) * SpotLights[index].Color * SpotLights[index].Intensity * lightAttenuation/* * inShadow*/);
